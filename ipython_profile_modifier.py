...
from traitlets.config.loader import PyFileConfigLoader, Config

def setup_ipython_profile(
    load_ext: str, exec_lines: str,
    ipython_path: Optional[str] = "", profile_name: str = "",
) -> bool:
    """Update or create an IPython profile to use the icat backend."""
    profile_path = get_profile_path(profile_name, ipython_path)

    extensions_line, exec_lines_line = dynamic_update_config(profile_path, load_ext, exec_lines)
...
def get_profile_path(profile_name: str, ipython_path: Optional[str | Path] = None) -> Path:
...
def dynamic_update_config(profile_path: Path, load_ext: str, exec_lines: str) -> Tuple[str, str]:
    ...
    _extensions = config.get("InteractiveShellApp", {}).get("extensions", [])
    _exec_lines = config.get("InteractiveShellApp", {}).get("exec_lines", [])

    if load_ext not in _extensions:
        _extensions.append(load_ext)

    if exec_lines not in _exec_lines:
        _exec_lines.append(exec_lines)

    extensions_line = f"c.InteractiveShellApp.extensions = {_extensions}\n"
    exec_lines_line = f"c.InteractiveShellApp.exec_lines = {_exec_lines}\n"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Setup IPython with required configurations")
    subparsers = parser.add_subparsers(dest="command")

    setup_parser = subparsers.add_parser("setup", help="Configure IPython to load tensorboard")
    setup_parser.add_argument(
        "load_ext", nargs=1, help="Name of the extension to load"
    )
    setup_parser.add_argument(
        "exec_lines", nargs=1, help="Command to execute", type=str
    )
    setup_parser.add_argument(
    ...
    if args.command == "setup":
        try:
            success = setup_ipython_profile(args.load_ext[0], args.exec_lines[0], args.ipython_path, args.profile)
            if not success:
                sys.exit(1)
        except Exception as e:
            print(f"Error: {e}")
